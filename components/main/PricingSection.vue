    ...
    <div class="relative isolate mt-32 bg-white px-6 sm:mt-56 lg:px-8">
        ...
        <div
            ...
            <div v-for="(tier, tierIdx) in tiers" :key="tier.id"
                ...
                <h3 :id="tier.id"
                    :class="[tier.featured ? 'text-blue-400' : 'text-blue-600', 'text-base font-semibold leading-7']">
                    {{ tier.name }}</h3>
                ...
                <ul role="list"
                    ...
                    <li v-for="feature in tier.features" :key="feature" class="flex gap-x-3">
                        <CheckIcon :class="[tier.featured ? 'text-blue-400' : 'text-blue-600', 'h-6 w-5 flex-none']"
                            aria-hidden="true" />
                        {{ feature }}
                    </li>
                </ul>
                <a :href="tier.href" :aria-describedby="tier.id"
                    :class="[tier.featured ? 'bg-blue-500 text-white shadow-sm hover:bg-blue-400 focus-visible:outline-blue-500' : 'text-blue-600 ring-1 ring-inset ring-blue-200 hover:ring-blue-300 focus-visible:outline-blue-600', 'mt-8 block rounded-md py-2.5 px-3.5 text-center text-sm font-semibold focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 sm:mt-10']">Get
                    started today</a>
            </div>
        </div>
    </div>
